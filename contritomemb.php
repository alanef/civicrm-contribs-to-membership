...
// this query handles expired memberships that have paid in last 2 years ( hard coded ) to handle specific  year end grace gone to expired before contributions logged
$query_expiredGraceMemberships = "UPDATE civicrm_membership AS m
inner join civicrm_membership_status ms on ms.id = m.status_id AND ms.name = 'Expired'
inner join civicrm_contact c on m.contact_id = c.id
inner join civicrm_contribution cn on c.id = cn.contact_id AND  YEAR(cn.receive_date) IN ( YEAR(CURDATE()) -2, YEAR(CURDATE()) -1 )
SET m.end_date = DATE_ADD(DATE_SUB(CURDATE(), INTERVAL DAYOFYEAR(CURDATE()) DAY), INTERVAL 1 YEAR),
m.status_id = 3";


$rsexpiredGraceMemberships = mysqli_query( $CiviCRM, $query_expiredGraceMemberships ) or die( mysqli_error( $CiviCRM ) );
$log->lwrite( "Expired moved to Grace Membership " );

// this query handles renewing grace memberships that has a contribution this year
...
$log->lwrite( "Grace Membership moved to Current " );
